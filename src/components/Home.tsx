...
const filterWithFilter: TFilterMethod = (movieState, filterState) => {
    if (filterState.yearRange !== "") {
        ...
        movieState = movieState.filter(
            (movie) =>
                movie.release_year >= bounds[0] &&
                movie.release_year <= bounds[1]
        );
    }
...
export default function Home() {
    ...
    useEffect(() => {
        if (search) {
            const filteredMovies = filterWithSearch(
                initialMovies.current,
                search
            );
            setMovies(filteredMovies);
        } else if (!Object.values(filter).every((val) => val === "")) {
            const filteredMovies = filterWithFilter(
                initialMovies.current,
                filter
            );
            setMovies(filteredMovies);
    ...
    return (
        <>
            ...
            {!isSmallScreen && modalOpen && selectedMovie && (
                <Modal {...selectedMovie} setModalState={setModalOpen} />
            )}
            {movies.length > 0 ? (
                <>
                                ...
                                return (
                                    <li>
                                        <MovieItem
                                            key={index}
                                            {...item}
                                            isInWatchlist={titlesInWatchlist.includes(
                                                item.title
                                            )}
